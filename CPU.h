...
private: //OPCODES
    ...
    /*  00+  */              OPCODE NOP();                  OPCODE LD_BC_nn(uint16_t nn); OPCODE LD_Addr_BC_A();        OPCODE INC_BC();           OPCODE INC_B();                 OPCODE DEC_B();        OPCODE LD_B_n(uint8_t n);       OPCODE RLCA();        OPCODE LD_Addr_nn_SP(uint16_t nn); OPCODE ADD_HL_BC(); OPCODE LD_A_Addr_BC();            OPCODE DEC_BC();          OPCODE INC_C();                OPCODE DEC_C();              OPCODE LD_C_n(uint8_t n);  OPCODE RRC_A();
