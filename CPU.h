...
private: //OPCODES
    ...
    /*  00+  */              OPCODE NOP();                  OPCODE LD_BC_nn(uint16_t nn); OPCODE LD_AddrBC_A();         OPCODE INC_BC();           OPCODE INC_B();                 OPCODE DEC_B();        OPCODE LD_B_n(uint8_t n);       OPCODE RLC_A();        OPCODE LD_Addr_nn_SP(uint16_t nn); OPCODE ADD_HL_BC(); OPCODE LD_A_Addr_BC();            OPCODE DEC_BC();    OPCODE INC_C();                OPCODE DEC_C();              OPCODE LD_C_n(uint8_t n);  OPCODE RRC_A();
    /*  10+  */              OPCODE STOP();                 OPCODE LD_DE_nn(uint16_t nn); OPCODE LD_Addr_DE_A();        OPCODE INC_DE();           OPCODE INC_D();                 OPCODE DEC_D();        OPCODE LD_D_n(uint8_t n);       OPCODE RL_A();         OPCODE JR_n(uint8_t n);            OPCODE ADD_HL_DE(); OPCODE LD_A_Addr_DE();            OPCODE DEC_DE();    OPCODE INC_E();                OPCODE DEC_E();              OPCODE LD_E_n(uint8_t n);  OPCODE RR_A();
    /*  20+  */              OPCODE JR_NZ_n(uint8_t n);     OPCODE LD_HL_nn(uint16_t nn); OPCODE LDI_Addr_HL_A();       OPCODE INC_HL();           OPCODE INC_H();                 OPCODE DEC_H();        OPCODE LD_H_n(uint8_t n);       OPCODE DAA();          OPCODE JR_Z_n(uint8_t n);          OPCODE ADD_HL_HL(); OPCODE LDI_A_Addr_HL();           OPCODE DEC_HL();    OPCODE INC_L();                OPCODE DEC_L();              OPCODE LD_L_n(uint8_t n);  OPCODE CPL();
    /*  30+  */              OPCODE JR_NC_n(uint8_t n);     OPCODE LD_SP_nn(uint16_t nn); OPCODE LDD_Addr_HL_A();       OPCODE INC_SP();           OPCODE INC_Addr_HL();           OPCODE DEC_Addr_HL();  OPCODE LD_Addr_HL_n(uint8_t n); OPCODE SCF();          OPCODE JR_C_n(uint8_t n);          OPCODE ADD_HL_SP(); OPCODE LDD_A_Addr_HL();           OPCODE DEC_SP();    OPCODE INC_A();                OPCODE DEC_A();              OPCODE LD_A_n(uint8_t n);  OPCODE CCF();
    ...
    /*  C0+  */              OPCODE RET_NZ();               OPCODE POP_BC();              OPCODE JP_NZ_nn(uint16_t nn); OPCODE JP_nn(uint16_t nn); OPCODE CALL_NZ_nn(uint16_t nn); OPCODE PUSH_BC();      OPCODE ADD_A_n(uint8_t n);      OPCODE RST_00h();      OPCODE RET_Z();                    OPCODE RET();       OPCODE JP_Z_nn(uint16_t nn);      OPCODE PREFIX_CB(); OPCODE CALL_Z_nn(uint16_t nn); OPCODE CALL_nn(uint16_t nn); OPCODE ADC_A_n(uint8_t n); OPCODE RST_08h();
    ...
    /*  F0+  */              OPCODE LD_A_FF00_n(uint8_t n)l OPCODE POP_AF();              OPCODE LD_A_FF00_C();         OPCODE DI();               /*         No Mapping        */ OPCODE PUSH_AF();      OPCODE OR_A_n(uint8_t n);       OPCODE RST_30h();      OPCODE LD_HL_SP_i(int8_t i);       OPCODE LD_SP_HL();  OPCODE LD_A_Addr_nn(uint16_t nn); OPCODE EI();        /*       No Mapping         */ /*       No Mapping       */ OPCODE CP_A_n(uint8_t n);  OPCODE RST_38h();
